...
def check_coupen(request, grand_total):
    ...
    if request.method == "POST":
        ...
        try:
            offer = Coupen.objects.get(
                coupen_code=coupen_code, is_active=True, coupen_count__gte=1
            )
            print(offer)
            if RedeemedCoupen.objects.filter(
                coupen_id=offer.id, user=request.user
            ):

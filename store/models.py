...
class Variation(models.Model):
    product = models.ForeignKey(
        product, on_delete=models.CASCADE, related_name="varion"
    )
    ...
    color = models.ForeignKey(
        VarientColor, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    def offer_price(self):
        ...
        try:
            if self.variationoffer.is_valid:
                ...
                return {
                    "new_price": new_price,
                    "discount": self.variationoffer.offer,
                }
        except:
            try:
                if self.product.productoffer.is_valid:
                    discount = (
                        self.price * self.product.productoffer.offer
                    ) / 100
                    new_price = self.price - discount
                    return {
                        "new_price": new_price,
                        "discount": self.product.productoffer.offer,
                    }
                raise
            except:
                try:
                    if self.product.vendor.productoffer.is_valid:
                        discount = (
                            self.price * self.product.vendor.productoffer.offer
                        ) / 100
                        new_price = self.price - discount
                        return {
                            "new_price": new_price,
                            "discount": self.product.vendor.productoffer.offer,
                        }
                    raise
                except:
                    try:
                        if self.product.category.productoffer.is_valid:
                            discount = (
                                self.price
                                * self.product.category.productoffer.offer
                            ) / 100
                            new_price = self.price - discount
                            return {
                                "new_price": new_price,
                                "discount": self.product.category.productoffer.offer,
                            }
                        raise
                    except:
                        return None

...
def shop(request, category_slug=None):
    ...
    if category_slug != None:
        ...
        varients = Variation.objects.all().filter(
            product__category=categories, is_available=True
        )
        print(varients)
...
def product_detail(request, category_slug, product_slug, varient_slug):
    try:
        ...
        in_cart = CartItem.objects.filter(
            cart__cart_id=_cart_id(request), varient=single_varient
        ).exists()
        print(in_cart)
    ...
    try:
        ordered = OrderProduct.objects.filter(
            user=request.user, variation=single_varient
        ).exists()
    except:
        ordered = None
    ...
    if "keyword" in request.GET:
        ...
        if keyword:
            products = Variation.objects.order_by("-created_at").filter(
                ...
                | Q(varient_name__icontains=keyword)
                | Q(product__vendor__brand_name__icontains=keyword)
            )
    ...
    if request.method == "POST":
        try:
            review = ReviewRating.objects.get(
                user__id=request.user.id, varient__id=varient_id
            )
